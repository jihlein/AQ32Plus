...
static uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum)
{
  ...
  if (USB_Tx_State == 1)
  {
    if (APP_Rx_length == 0)
    {
     ...
     /* Transmit zero sized packet in case the last one has maximum allowed size. Otherwise   // HJI Patch from STM Forum, Tags: stm32 usb library cdc device !bug
      * the recipient may expect more data coming soon and not return buffered data to app.   // HJI Patch from STM Forum, Tags: stm32 usb library cdc device !bug
      * See section 5.8.3 Bulk Transfer Packet Size Constraints                               // HJI Patch from STM Forum, Tags: stm32 usb library cdc device !bug
      * of the USB Specification document.                                                    // HJI Patch from STM Forum, Tags: stm32 usb library cdc device !bug
      */                                                                                      // HJI Patch from STM Forum, Tags: stm32 usb library cdc device !bug
