...
int MAG_Grid(MAGtype_CoordGeodetic minimum, MAGtype_CoordGeodetic maximum, float
        cord_step_size, float altitude_step_size, float time_step, MAGtype_MagneticModel *MagneticModel, MAGtype_Geoid
...
INPUT: minimum :Data structure with the following elements (minimum limits of the grid)
                ...
                step_size  : float  : spatial step size, in decimal degrees
                a_step_size : float  :  float altitude step size (km)
                step_time : float  : time step size (decimal years)
...
int MAG_GetUserGrid(MAGtype_CoordGeodetic *minimum, MAGtype_CoordGeodetic *maximum, float *step_size, float *a_step_size, float *step_time, MAGtype_Date
...
UPDATE : minimum Pointer to data structure with the following elements
                ...
                step_size  : float pointer : spatial step size, in decimal degrees
                a_step_size : float pointer :  float altitude step size (km)
                step_time : float pointer : time step size (decimal years)
 ...
 */
{
    ...
    if(Geoid->UseGeoid == 1)
    {
        ...
        fgets(buffer, 20, stdin);
        sscanf(buffer, "%f", &minimum->HeightAboveGeoid);
        strcpy(buffer, "");
        printf("Please Enter Maximum Height above MSL (in km):\n");
        fgets(buffer, 20, stdin);
        sscanf(buffer, "%f", &maximum->HeightAboveGeoid);
        strcpy(buffer, "");
        printf("Please Enter height step size (in km):\n");
        fgets(buffer, 20, stdin);
        sscanf(buffer, "%f", a_step_size);
        strcpy(buffer, "");
    } else
    {
    ...
    fgets(buffer, 20, stdin);
    sscanf(buffer, "%f", &StartDate->DecimalYear);
    strcpy(buffer, "");
    printf("Please Enter the decimal year ending time:\n");
    fgets(buffer, 20, stdin);
    sscanf(buffer, "%f", &EndDate->DecimalYear);
    strcpy(buffer, "");
    printf("Please Enter the time step size:\n");
    fgets(buffer, 20, stdin);
    sscanf(buffer, "%f", step_time);
    strcpy(buffer, "");
 ...
 */
{
    ...
    for(i = 0, done = 0, j = 0; i <= 40 && !done; i++)
    {
        if(buffer[i] == '.')
        {
            j = sscanf(buffer, "%f", &CoordGeodetic->phi);
            if(j == 1)
                done = 1;
            else
                done = -1;
        ...
        if(buffer[i] == '\0' || done == -1)
        {
            ...
            {
                sscanf(buffer, "%f", &CoordGeodetic->phi);
                done = 1;
            } else
            {
    ...
    {
        if(buffer[i] == '.') /*This detects if there is a '.' somewhere in the string, if there is the program tries to interpret the input as a float, and copies it to the longitude*/
        {
            j = sscanf(buffer, "%f", &CoordGeodetic->lambda);
            if(j == 1)
        ...
        if(buffer[i] == '\0' || done == -1) /*If the program reaches the end of the string before finding a "." or a "," or if its been sent by an error it does this*/
        {
            ...
            {
                sscanf(buffer, "%f", &CoordGeodetic->lambda);
                done = 1;
    ...
    while(!done)
    {
        ...
        {
            j = sscanf(buffer, "%c%f", &tmp, &CoordGeodetic->HeightAboveEllipsoid);
            if(j == 2)
                j = 1;
            Geoid->UseGeoid = 0;
        ...
        {
            Geoid->UseGeoid = 1;
            j = sscanf(buffer, "%f", &CoordGeodetic->HeightAboveGeoid);
    ...
    for(i = 0, done = 0; i <= 40 && !done; i++)
    {
        if(buffer[i] == '.')
        {
            j = sscanf(buffer, "%f", &MagneticDate->DecimalYear);
            if(j == 1)
                done = 1;
            else
                buffer[i] = '\0';
        ...
        if(buffer[i] == '/')
        {
            ...
            if(!MAG_DateToYear(MagneticDate))  // HJI if(!MAG_DateToYear(MagneticDate, Error_Message))
            {
...
INPUT :  GeomagElements : Data structure MAGtype_GeoMagneticElements with the following elements
                        ...
                        float Z; Downward component of the magnetic field vector4
...
/*Return value 0 means end program, Return value 1 means get new data, Return value 2 means continue.
  This prints a warning to the screen determined by the control integer. It also takes the value of the parameter causing the warning as a float.  This is unnecessary for some warnings.
  It requires the MagneticModel to determine the current epoch.

 INPUT control :int : (Warning number)
                value  : float: Magnetic field strength
                MagneticModel
OUTPUT : none
CALLS : none
...
int MAG_DateToYear(MAGtype_Date *CalendarDate)  // HJI int MAG_DateToYear(MAGtype_Date *CalendarDate, char *Error)
 ...
 */
{
    ...
    /******************Validation********************************/
    // HJI if(CalendarDate->Month <= 0 || CalendarDate->Month > 12)
    // HJI {
    // HJI     strcpy(Error, "\nError: The Month entered is invalid, valid months are '1 to 12'\n");
    // HJI     return 0;
    // HJI }
    // HJI if(CalendarDate->Day <= 0 || CalendarDate->Day > MonthDays[CalendarDate->Month])
    // HJI {
 ...
 */
{
    ...
    DayOfTheYear = floor((CalendarDate->DecimalYear - (float) CalendarDate->Year) * (365.0 + (float) ExtraDay)+0.5) + 1;
